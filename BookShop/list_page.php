		...
		<section>
			<?php
				...
				if (isset($_GET["search"]) || isset($_GET["genre"])) {
					...
					if (isset($_GET["genre"]))
						$query = 'SELECT bd.genre, bd.title, b.price FROM book_detail bd join book b on b.isbn=bd.isbn WHERE genre in(\'%'.$_GET["search"].'%\')';
					else
						$query = 'SELECT bd.genre, bd.title, b.price FROM book_detail bd join book b on b.isbn=bd.isbn WHERE lower(title) like lower(\'%'.$_GET["search"].'%\')';
